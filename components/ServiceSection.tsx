...
export function ServiceColumn({
  ...
  return (
    <motion.div
      ...
      onHoverStart={() => typeof window !== "undefined" && window.innerWidth >= 768 && setHovered(true)}
      onHoverEnd={() => setHovered(false)}
      className="
        relative flex flex-col cursor-default bg-black
        w-full rounded-[18px] border border-white/10 p-5 gap-3
        
        /* Removed overflow-hidden for small screens, kept strictly for md and above */
        md:overflow-hidden md:p-0 md:gap-0 md:h-auto md:flex-1 md:rounded-none md:border-0
      "
      style={{
        borderRight:
          index < total - 1 && typeof window !== "undefined" && window.innerWidth >= 768
    ...
    >
      ...
      >
        ...
        {/* Mobile image dimming layer */}
        <div className="absolute inset-0 bg-black/35 md:hidden" />

        {/* Desktop styling linear gradient blends */}
        <div
          className="absolute inset-x-0 bottom-0 hidden md:block"
          style={{
            height: "100%",
            background: "linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.65) 35%, rgba(0,0,0,0.9) 80%, #000 100%)",
            pointerEvents: "none",
          }}
        />
        <div
      ...
      <div
        className="
          ...
          /* Absolute multi-stage breakpoint settings for desktop display scales */
          md:absolute md:inset-x-0 md:z-20 md:h-[40vh] md:p-0
          md:bottom-[-12px]   /* Tablets / Medium screens */
          lg:bottom-[20px]    /* Normal Desktop Laptops */
          xl:bottom-[48px]    /* Large Display Monitors */
        "
        style={{ 
          pointerEvents: typeof window !== "undefined" && window.innerWidth < 768 ? "auto" : "none" 
        }}
      >
        {/* Mobile Keyword Label Stamp */}
        <span className="mb-1 block font-mono text-[8px] uppercase tracking-[0.26em] text-[#c8a052] md:hidden">
          {service.keyword}
        </span>

        {/* Title Container Block layout */}
        <div
        ...
        {/* Description body copy and detail list tags */}
        <div className="mt-2 flex flex-col gap-4 md:mt-0 md:gap-2 md:pt-3">
          ...
          <div className="grid grid-cols-1 gap-y-2 mt-1 md:flex md:flex-col md:gap-1.5">
  ...
  return (
    <>
      <motion.section
        ...
        className="flex flex-col overflow-y-auto overflow-x-hidden md:overflow-hidden"
      ...
      >
        ...
        <div
          className="
            flex min-h-0 flex-col gap-6 px-4 pt-16 pb-32
            md:flex-1 md:flex-row md:gap-0 md:px-0 md:pb-0 md:pt-0
          "
        >
        ...
        <div className="md:hidden fixed bottom-6 right-6 z-50">
          ...
          >
            {/* Minimalist modern graphic layout configuration representing fortune/growth */}
