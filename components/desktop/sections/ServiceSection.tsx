...
// ─────────────────────────────────────────────
// Hint Toast — "Click on services to check the work"
// Auto-dismisses after 4 s, manual close button
// ─────────────────────────────────────────────
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="pointer-events-auto fixed bottom-6 left-1/2 z-[150] -translate-x-1/2"
        ...
        >
          <div
            ...
            style={{
              ...
              border: "0.5px solid rgba(200,160,80,0.25)",
          ...
          >
            ...
            <span
              ...
              style={{ background: "#c8a052" }}
            />

            <span
              className="font-mono text-[9px] uppercase tracking-[0.22em] whitespace-nowrap"
              style={{ color: "rgba(232,216,184,0.78)" }}
            >
              Click on a service to explore the work
            </span>

            {/* Close */}
            <button
              onClick={onClose}
              className="ml-1 flex h-4 w-4 shrink-0 items-center justify-center rounded-full transition hover:bg-white/10"
              style={{
                background: "none",
                border: "none",
                cursor: "pointer",
...
export const ServiceColumn = forwardRef<HTMLDivElement, ServiceColumnProps>(
  ({ service, index, total, isActive, onClick }, ref) => {
    ...
    return (
      <motion.div
        ...
        className={
          `
          snap-start md:snap-none min-h-[80vh] md:min-h-0 relative flex flex-col cursor-pointer bg-black
...
function MobileServicesFlow({ onOpenOverlay }: { onOpenOverlay: () => void }) {
  ...
  // Hint toast: show when heading comes into view, auto-dismiss after 4 s
  ...
  // 💡 Defer the state update to prevent cascading synchronous renders
  queueMicrotask(() => {
    setShowHint(true);
  });
  ...
  return (
    <>
      <HintToast visible={showHint} onClose={dismissHint} />
        ...
        <div className="flex flex-col gap-4 px-4 pb-6">
          {SERVICES.map((service, i) => (
            <MobileServiceCard
              ...
              service={service}
              index={i}
              onClick={() => router.push(`/work/services/${service.slug}`)}
            />
          ))}
        </div>
...
function MobileServiceCard({ service, index, onClick }: { service: Service; index: number; onClick: () => void }) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="cursor-pointer rounded-[14px] overflow-hidden border border-white/10 bg-black active:scale-[0.98] transition-transform duration-150"
    >
      {/* Text block */}
...
export default function ServicesSection({
  ...
  // Show hint as soon as the section becomes active on desktop
  useEffect(() => {
  if (isMobileFlow) return;
  if (!isActive) return;

  // 💡 Defer execution to clear the synchronous layout cycle
  queueMicrotask(() => {
    handleShowHint();
  });
  ...
  return (
    <>
      {/* Hint toast for desktop — rendered outside motion.section so it isn't clipped */}
      <HintToast visible={showHint} onClose={dismissHint} />
