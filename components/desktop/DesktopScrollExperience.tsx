...
function getTimeline(mode: ViewportMode): Timeline {
  ...
  return {
    ...
    active: {
      ...
      landingEnd: 0.26,
      servicesEnd: 0.38,
      howWeWorkEnd: 0.52,   // widened from 0.50 — gives snap more room
      workEnd: 0.88,
    },
    ...
    howWeWork: {
      opacity: [0.38, 0.42, 0.44, 0.50],  // trigger at 0.44 — earlier than before
      y: [0.38, 0.42],
      exitY: [0.46, 0.52],
      target: 0.43,
    },
    ...
    work: {
      x: [0.50, 0.56],                     // shifted to match new howWeWorkEnd
      opacity: [0.50, 0.56, 0.84, 0.87],
      headingY: [0.52, 0.56],
      headingScale: [0.54, 0.58],
      headingX: [0.54, 0.58],
      carouselOpacity: [0.56, 0.58],
      carouselX: [0.61, 0.84],
      exitY: [0.84, 0.88],
      target: 0.59,
    },
...
export default function ScrollExperience() {
  ...
  // ─────────────────────────────────────────────────────────────────
  // Single consolidated scroll event handler
  // All four previous handlers merged into one to avoid 4× re-renders
  // per scroll tick. State updates are batched via refs where possible.
  // ─────────────────────────────────────────────────────────────────
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest: number) => {
    ...
    nudgeTimerRef.current = setTimeout(() => {
      if (prevActiveRef.current !== "contact") setShowNudge(true);
    }, 2000);

    // ── 4. Auto-snap ───────────────────────────────────────────────
    if (isAnimatingRef.current) return;
    if (isSnappingRef.current) return;
    if (isMobile) return; // mobile uses normal scroll, no snapping

    if (!hasSettledOnLandingRef.current) {
      if (latest >= timeline.landing.target) {
    ...
    if (!isScrollingDown) {
      // ── Upward snaps ──
      const landingBackThreshold   = timeline.services.opacity[1];
      const servicesBackThreshold  = timeline.howWeWork.opacity[1];
      const howWeWorkBackThreshold = timeline.work.opacity[1];
      const workBackThreshold      = timeline.contact.opacity[1];

      if (previous >= landingBackThreshold && latest <= landingBackThreshold && latest > timeline.landing.target && !autoScrollTriggeredRef.current.has("landing-back")) {
        performSnapTo(timeline.landing.target, "landing-back", 1200); return;
      }
      if (previous >= servicesBackThreshold && latest <= servicesBackThreshold && latest > timeline.services.target && !autoScrollTriggeredRef.current.has("services-back")) {
        performSnapTo(timeline.services.target, "services-back", 1200); return;
      }
      if (previous >= howWeWorkBackThreshold && latest <= howWeWorkBackThreshold && latest > timeline.howWeWork.target && !autoScrollTriggeredRef.current.has("howwework-back")) {
        performSnapTo(timeline.howWeWork.target, "howwework-back", 1200); return;
      }
      if (previous >= workBackThreshold && latest <= workBackThreshold && latest > timeline.work.target && !autoScrollTriggeredRef.current.has("work-back")) {
        performSnapTo(timeline.work.target, "work-back", 1200); return;
      }
    ...
    if (!autoScrollTriggeredRef.current.has("landing") && latest >= timeline.landing.opacity[1] && latest < timeline.active.landingEnd) {
      performSnapTo(timeline.services.target, "landing", 1500); return;
    }
    if (!autoScrollTriggeredRef.current.has("services") && latest >= timeline.services.opacity[2] && latest < timeline.active.servicesEnd) {
      performSnapTo(timeline.howWeWork.target, "services", 1500); return;
    }
    if (!autoScrollTriggeredRef.current.has("howwework") && latest >= timeline.howWeWork.opacity[2] && latest < timeline.active.howWeWorkEnd) {
      performSnapTo(timeline.work.target, "howwework", 1500); return;
    }
    if (!autoScrollTriggeredRef.current.has("work") && latest >= timeline.work.exitY[0] && latest < timeline.active.workEnd) {
      performSnapTo(timeline.contact.target, "work", 1500); return;
    }
  });
          ...
          <div className="sticky top-0 h-svh overflow-hidden bg-[#f5f2eb]">
            ...
            {showNudge && activeSegment !== "contact" && (
              ...
              >
                ...
                <span
                  className="
                    font-mono uppercase text-[15px] tracking-[0.32em]
                  "
