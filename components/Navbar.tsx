...
export default function Navbar({ showNavbar }: NavbarProps) {
  return (
    <motion.header
      initial={false}
      animate={showNavbar ? { opacity: 1, y: 0 } : { opacity: 0, y: -50 }}
      transition={{ duration: 0.35, ease: "easeInOut" }}
      className="pointer-events-auto fixed left-0 top-0 z-[999] w-full border-b border-black/10 bg-white/50 backdrop-blur-md"
    >
