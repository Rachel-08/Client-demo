...
export default function ScrollExperience() {
  ...
  const handleEnterClick = useCallback((): void => {
    scrollToProgress(timeline.intro.enterTarget);
  }, [scrollToProgress, timeline.intro.enterTarget]);
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest: number) => {
    if (latest < timeline.active.introEnd) {
      setActiveSegment("intro");
    } else if (latest < timeline.active.landingEnd) {
      setActiveSegment("landing");
    } else if (latest < timeline.active.servicesEnd) {
      setActiveSegment("services");
    } else if (latest < timeline.active.howWeWorkEnd) {
      setActiveSegment("howwework");
    } else if (latest < timeline.active.workEnd) {
  ...
  const handleNavigate = useCallback(
    (target: NavigateTarget) => {
      ...
      scrollToProgress(progressMap[target]);
    },
    [
      scrollToProgress,
  ...
  const landingExitX = useTransform(
    ...
    ["0vw", viewportMode === "mobile" ? "-170vw" : "-140vw"]
  );
  ...
  const howWeWorkY = useTransform(
    ...
    [viewportMode === "mobile" ? "65vh" : "55vh", "0vh"]
  );

  const howWeWorkExitY = useTransform(
    scrollYProgress,
    timeline.howWeWork.exitY,
    ["0vh", viewportMode === "mobile" ? "-70vh" : "-55vh"]
  );
  ...
  const contactY = useTransform(
    scrollYProgress,
    timeline.contact.y,
    [viewportMode === "mobile" ? "65vh" : "50vh", "0vh"]
  );
  ...
  return (
    <>
      ...
      >
        <div className="sticky top-0 h-[100svh] overflow-hidden bg-[#f5f2eb]">
          <Navbar
        ...
        <ContactSection
          ...
          {showNudge && activeSegment !== "contact" && (
            ...
            >
              {[0, 1, 2].map((i) => (
                ...
                >
                  <svg
                    viewBox="0 0 16 9"
                    fill="none"
                    className="h-[12px] w-[16px] md:h-[18px] md:w-[20px]"
                  >
                    <polyline
                      points="1,1 8,8 15,1"
              ...
              <span
                className="
                  font-mono uppercase text-[9px] tracking-[0.28em]
                  md:text-[15px] md:tracking-[0.32em]
                "
